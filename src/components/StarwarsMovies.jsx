...
function StarwarsMovies() {
  ...
  async function gitMovieList(page) {
    let moviesResponse = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=c636ed7787cc302d96bf88ccf334e0d8&language=en-US&query=star%20wars&include_adult=false&page=${page}`)
    setListMovies([...listMovies, ...moviesResponse.data.results])
  }
      ...
      {listMovies ?
        <Content>
          {listMovies.map((movie, key) =>
            <Wrap key={key}>
              <Link to={`/disney-plus/${'movie'}/details/${movie.id}`}>
